...
pub const sine_table = struct {
    // 65,536 entries covering [0, 2π), computed at compile time. This moves both
    // the @sin computation and the 256 KiB of storage out of runtime: generation
    // previously recomputed every entry per world (a soft-float sin on the PSP)
    // and carried the table as a stack local, which overflowed the PSP's default
    // 256 KiB thread stack. The comptime values are bit-identical to the runtime
    // computation, so carving decisions are unchanged.
    pub const entries: [65_536]f32 = blk: {
    ...
    /// Empty handle; the entries live in comptime (.rodata) storage.
    pub fn init() sine_table {
        return .{};
    }

    pub fn trajectory_sine(self: *const sine_table, angle: f32) f32 {
        _ = self;
    ...
    pub fn trajectory_cosine(self: *const sine_table, angle: f32) f32 {
        _ = self;
        const scaled: f32 = angle * @as(f32, 10_430.378);
...
pub fn advanced_motion(table: *const sine_table, motion: motion_32) motion_32 {
    const horizontal = table.trajectory_cosine(motion.pitch);
    const x_delta: f32 = table.trajectory_sine(motion.yaw) * horizontal;
    const z_delta: f32 = table.trajectory_cosine(motion.yaw) * horizontal;
    return .{
        .position = .{
            .x = motion.position.x + x_delta,
            .y = motion.position.y + table.trajectory_sine(motion.pitch),
...
fn coordinate_bounds(center: f32, radius: f32, dimension: u32) struct { first: u32, end: u32 } {
    ...
    // `center - extent` is already computed in f32 before its (exact) widening,
    // so @floor of the widened f64 value is the same integer as floor_f32 of
    // the f32 difference. The f64 form only ran soft-float libcalls on the PSP
    // (every cave/ore ellipsoid calls bounds 3 times); the f32 form is
    // bit-identical and libcall-free.
...
/// One passing (y, z) row of the ellipsoid walk: the exact per-cell
/// predicate of the box scan, walking x outward from the nearest column.
/// Kept as a module-level function so it can also serve the resource passes.
fn walk_ellipsoid_row(
    ...
    const dz: f32 = @as(f32, @floatFromInt(z)) - center_z;
    const dz_squared: f32 = dz * dz;
    const row_base = walk_field.dimensions.index(x_first, y, z);
    // Walk down from the nearest x; each step tests the exact per-cell
    // predicate and writes the same cells a box scan would. Row base + 0 is
    // the first box cell (x == first). Keeping the block and memo indices as
    // running pointers removes the per-cell re-derived offsets.
    ...
    // Walk up from the next x after the center-side cell above.
...
pub fn replace_stone_ellipsoid(field: terrain.block_field, center: point_32, radius: f32, material: u8) void {
    ...
    // The metric components depend on single coordinates (dx^2, 2*dy^2, dz^2),
    // and the block layout is x-major. Each axis extent is bounded by the
    // cave/ore radius formulas (|radius| < 5.7 for caves, < 1.9 for veins),
    // so a 64-entry memo always fits; the fallback below re-evaluates dx per
    // cell and is kept only for API safety against future radius growth — it
    // computes the same values.
    const x_count = x_bounds.end - x_bounds.first;
    const y_count = y_bounds.end - y_bounds.first;
    const z_count = z_bounds.end - z_bounds.first;
    var dx_squared_memo: [offset_memo_max]f32 = undefined;
    const memoized = x_count <= offset_memo_max and y_count <= offset_memo_max and
        z_count <= offset_memo_max;
    if (memoized) {
    ...
    if (memoized) {
        // For a fixed (y, z) row, metric(x) == (dx^2 + 2*dy^2) + dz^2 is
        // minimized at the integer x nearest to center.x (float squaring and
        // addition are monotone, so the grouping above is monotone in |dx|).
        // If even that minimum fails the radius test, no cell in the row can
        // pass — skip the whole row. Rows that pass walk outward from the
        // nearest x, testing exactly the cells a box scan would accept plus
        // one rejection per side (rejections are monotone, so the walk stops
        // at the first). The same monotonicity holds in y and z (2*dy^2 and
        // dz^2 grow with |dy| and |dz|), so the enclosing loop walks y and z
        // outward from the nearest positions and stops at the first wholly
        // failing row: it tests exactly the (y, z) rows a box scan would
        // consider, plus one rejection per boundary, instead of the full
        // rectangle (an axis-aligned box scan costs all (y_count * z_count)
        // row tests even when the ellipsoid fills only ~pi/4 of them).
        // Every predicate keeps the exact grouping of the box scan
        // ((min_dx^2 + 2*dy^2) + dz^2 and (dx^2 + 2*dy^2) + dz^2, f32 adds
        // are commutative so the min_dx terms equal the per-cell ones at the
        // same point), and writes target disjoint cells, so the resulting
        ...
        // Walk y (and, per passing row, z) outward from the nearest position, (and, per passing row, z) outward from the nearest position,
        // stopping at the first wholly failing level. Each row body recomputes
        // nothing a box scan would not; the checks share the box scan's exact
        // (min_dx^2 + 2*dy^2) + dz^2 grouping.
        ...
        while (y_down >= y_range_start) : (y_down -= 1) {
            ...
            // Same grouping, the x+y part hoisted once per y for the z-walks.
            ...
            while (z_down >= z_range_start) : (z_down -= 1) {
                ...
                walk_ellipsoid_row(field, center.z, radius_squared, dy_squared_twice, z, y, x_bounds.first, material, &dx_squared_memo, x_range_start, x_range_end_excl, row_minimum_x);
            }
        ...
        for (x_bounds.first..x_bounds.end) |x_usize| {
            ...
            for (y_bounds.first..y_bounds.end) |y_usize| {
                ...
                const dy: f32 = @as(f32, @floatFromInt(y)) - center.y;
                const dy_squared: f32 = dy * dy;
                const horizontal_and_vertical: f32 = dx_squared + dy_squared * @as(f32, 2.0);
...
fn cave_radius(table: *const sine_table, dimensions: terrain.world_dimensions, seed: cave_seed, step: u32, center: point_32) f32 {
    ...
    return table.trajectory_sine(phase) * maximum;
}

pub fn cave_pass(table: *const sine_table, random: *random_state, field: terrain.block_field) void {
    const dimensions = field.dimensions;
    ...
    for (0..@as(usize, @intCast(attempts))) |_| {
        ...
        for (0..seed.length) |step_usize| {
            ...
            const moved = advanced_motion(table, motion);
            ...
            if (decision >= 0.25) {
                ...
                const radius = cave_radius(table, dimensions, seed, step, center);
...
test "sine lookup has specified phase endpoints" {
    const table = sine_table.init();
    try std.testing.expectEqual(@as(f32, 0.0), sine_table.entries[0]);
    try std.testing.expectApproxEqAbs(@as(f32, 1.0), sine_table.entries[16_384], 0.000001);
    try std.testing.expectEqual(sine_table.entries[0], table.trajectory_sine(0.0));
    try std.testing.expectEqual(sine_table.entries[16_384], table.trajectory_cosine(0.0));
}
...
test "zero-volume cave count preserves field and state" {
    ...
    const table = sine_table.init();
    cave_pass(&table, &random, field);
    try std.testing.expectEqual(state, random.state);
}

test "nonzero cave pass is deterministic and only removes stone" {
    ...
    var first_random = random_state.init(-300);
    var second_random = random_state.init(-300);
    const table = sine_table.init();
    cave_pass(&table, &first_random, first_field);
    cave_pass(&table, &second_random, second_field);
